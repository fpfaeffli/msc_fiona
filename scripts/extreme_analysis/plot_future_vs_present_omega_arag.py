...
for config in configs:
    ...
    for scenario in ['present']:  # ,'ssp245','ssp585'
        ...
        print(f'{config}, {scenario}')
        threshold, threshold_366 = ModelGetter.get_threshold('omega_arag_offl', 0, 'relative', 5, config, scenario)  # variable, depth_level, threshold_type, threshold_value, config, scenario
...
fig.suptitle(f'Days per year of extreme surface omega aragonite saturation based on present-day threshold (fixed baseline) (ensemble {ensemble_run})', fontsize=16, y=1.02)
...
# Set the figure title 
fig.suptitle(f'Days per year of extreme surface omega aragonite saturation based on present_plus_meandelta threshold (moving baseline) (ensemble {ensemble_run})', fontsize=16, y=1.02)
plt.subplots_adjust(top=0.85, right=0.9)
