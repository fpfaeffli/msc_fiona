...
# Parameters specific to 'present'
vmax_large = 300
vmax_small = 25
...
# Set the figure title for 'present'
...
# Axis limits and ticks for 'present'
for axi in ax:
...
savedir = '/nfs/sea/work/fpfaeffli/plots/future_vs_present_Hplus/anomalies_rel_tresh/number_days/'
filename = f'future_vs_present_anomalies_rel_to_thresh_{threshold_type}_for_region_coastal_all_lats_map_number_of_days_romsoc_fully_coupled.png'
...
# TODO: change the y-axis range on the present_plus_meandelta plots
...
for region_cho in ['all_dists_all_lats']:
    for threshold_type in ['present', 'present_plus_meandelta']:  # ,'present_plus_climdelta']:
        ...
        if threshold_type == 'present_plus_meandelta':
            ...
            ax[0].set_ylim(0, 350) # New extremes
        ...
        if threshold_type == 'present_plus_meandelta':
            fig.suptitle(f'Extreme days per year based on present_plus_meandelta (ssp585) threshold', fontsize=16, y=1.02)
        else:
            fig.suptitle(f'Extreme days per year based on present-day threshold', fontsize=16, y=1.02)
        
        ax[0].legend() 
        plt.tight_layout() 

        # Save the figure
        # TODO: change the savedir
        savedir = '//nfs/sea/work/fpfaeffli/plots/future_vs_present_Hplus/anomalies_rel_tresh/number_days/'
        filename = f'future_vs_present_anomalies_rel_to_thresh_{threshold_type}_for_region_coastal_all_lats_map_number_of_days_romsoc_fully_coupled.png'
        # plt.savefig(savedir + filename, dpi=200, transparent=True)
